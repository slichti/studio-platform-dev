...
### Systematic Testing (Phase 21)
- **Framework**: Installed and configured **Playwright** in `apps/web`.
- **Infrastructure**:
    - Fixed database schema drift by regenerating `local.db` and Drizzle config.
    - Seeded deterministic test data (`test-studio`) using `seed:tenant` script.
- **Tests Created**:
    - `auth.spec.ts`: Signup and Login routing validation.
    - `booking.spec.ts`: Validates Schedule view, Class Cards, and Book button visibility for real seed data.
    - `challenges.spec.ts`: Checks Leaderboard or Auth redirect.
    - `portal.spec.ts`: Verifies student portal access control.
- **Results**: `npx playwright test` configured. `booking.spec.ts` verifies public schedule data flow.
