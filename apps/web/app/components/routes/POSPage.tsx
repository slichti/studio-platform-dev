        ...
        <div className="flex h-full">
            ...
            <div className="flex-1 p-4 overflow-y-auto bg-zinc-50 dark:bg-zinc-950">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h1 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">POS & Retail</h1>
                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-semibold">Inventory & Sales</p>
                    </div>
                    <div className="flex gap-2">
                        <button
                            onClick={() => setShowReaderModal(true)}
                            className={`flex items-center gap-2 px-2 py-1 rounded-md border text-xs transition-colors ${terminalStatus === 'connected' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-white text-zinc-700 border-zinc-200'}`}
                        >
                            <Wifi size={14} />
                            {terminalStatus === 'connected' ? activeReader?.label || 'Reader Connected' : 'Connect'}
                        </button>
                        <button
                            onClick={() => { setEditingProduct(null); setShowProductModal(true); }}
                            className="flex items-center gap-2 px-2 py-1 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-md text-xs hover:opacity-90"
                        >
                ...
                <div className="flex flex-col gap-2">
                    {products.map(product => (
                        <div key={product.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-2 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow group relative">
                            <div className="h-10 w-10 bg-zinc-100 dark:bg-zinc-800 rounded flex-shrink-0 flex items-center justify-center text-zinc-300">
                                {product.imageUrl ? <img src={product.imageUrl} className="w-full h-full object-cover rounded" /> : <Archive size={16} />}
                            </div>

                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <h3 className="font-semibold text-xs text-zinc-900 dark:text-zinc-100 truncate">{product.name}</h3>
                                    {product.type === 'membership' && (
                                        <span className="text-[8px] uppercase font-bold text-purple-600 bg-purple-50 px-1 rounded">Membership</span>
                                    )}
                                </div>
                                <div className="flex items-center gap-3">
                                    <p className="text-[10px] text-zinc-400 truncate">{product.category || 'General'}</p>
                                    <span className={`text-[10px] ${product.type === 'membership' ? 'text-zinc-300' : (product.stockQuantity > 0 ? 'text-green-600' : 'text-red-500')} font-medium`}>
                                        {product.type === 'membership' ? 'Subscription' : `${product.stockQuantity} in stock`}
                                    </span>
                                </div>
                            </div>

                            <div className="flex items-center gap-4 py-1 pr-1">
                                <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">${(product.price / 100).toFixed(2)}</span>
                                <div className="flex items-center gap-1">
                                    {product.type === 'product' && (
                                        <button
                                            onClick={() => { setEditingProduct(product); setShowProductModal(true); }}
                                            className="p-1 text-zinc-300 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity"
                                        >
                                            <Edit size={14} />
                                        </button>
                                    )}
            ...
            {/* --- Sidebar Cart --- */}
            <div className="w-72 bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 flex flex-col z-10 shadow-xl">
                <div className="p-4 border-b border-zinc-100 dark:border-zinc-800">
                    {selectedCustomer ? (
                        <div className="flex justify-between items-center bg-blue-50 dark:bg-blue-900/20 p-2 rounded border border-blue-100 dark:border-blue-800">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <div className="text-xs font-medium text-blue-900 dark:text-blue-100">{selectedCustomer.profile ? `${selectedCustomer.profile.firstName} ${selectedCustomer.profile.lastName}` : selectedCustomer.name}</div>
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-full flex items-center justify-between px-2 py-1.5 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded text-xs text-zinc-500 hover:border-blue-400 transition-colors"
                            >
                                <span className="flex items-center gap-2"><User size={12} /> Add Customer</span>
                                <Plus size={12} />
                            </button>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {cart.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center group py-0.5">
                            <div className="min-w-0">
                                <div className="text-xs font-medium text-zinc-900 dark:text-zinc-100 truncate">{item.name}</div>
                                <div className="text-[10px] text-zinc-500">${(item.price / 100).toFixed(2)} x {item.quantity}</div>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0">
                                <div className="font-semibold text-xs">${(item.price * item.quantity / 100).toFixed(2)}</div>
                    ...
                    {cart.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-400 space-y-1 opacity-50 py-10">
                ...
                <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <button
                            ...
                            className="bg-white border border-zinc-200 text-zinc-700 py-1.5 rounded-lg text-xs font-medium hover:bg-zinc-50 flex items-center justify-center gap-1.5 disabled:opacity-50"
                        >
                            <CreditCard size={12} /> Manual Card
                        </button>
                        <button
                            disabled={processing || cart.length === 0}
                            onClick={() => handleCheckout('cash')}
                            className="bg-green-600 text-white py-1.5 rounded-lg text-xs font-medium hover:bg-green-700 disabled:opacity-50"
                        >
                            Cash
                        </button>
                    </div>
                    <button
                        ...
                        className={`w-full py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 shadow-lg transition-all ${terminalStatus === 'connected' ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200 dark:shadow-blue-900/20' : 'bg-zinc-200 text-zinc-500 cursor-not-allowed'}`}
                    >
                        {processing ? <Loader2 className="animate-spin" size={16} /> : <Wifi size={16} />}
                        {terminalStatus === 'connected' ? 'Charge Application' : 'Connect Reader'}
                    </button>
                </div>
            </div>
