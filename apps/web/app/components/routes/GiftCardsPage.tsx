...
export default function GiftCardsPageComponent() {
    const { giftCards: initialCards, token } = useLoaderData<any>();
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
                                                            ...
                                                            <div className="relative">
                                                                ...
                                                                {activeMenuId === card.id && (
                                                                    <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-100 dark:border-zinc-800 z-50 overflow-hidden py-1">
                                                                        ...
                                                                        <button onClick={() => { setActiveMenuId(null); toast.info("Void feature coming soon"); }} className="w-full text-left px-4 py-2.5 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 text-red-600 flex items-center gap-2 border-t border-zinc-100 dark:border-zinc-800">
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
                    <div className="bg-white dark:bg-zinc-900 rounded-3xl p-8 max-w-md w-full shadow-2xl">
                        ...
                        <form onSubmit={handleLinkCard} className="space-y-4">
                            <input type="text" placeholder="GIFT-XXXX-XXXX" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800 rounded-xl font-mono uppercase" value={linkCode} onChange={(e) => setLinkCode(e.target.value.toUpperCase())} required />
                            <div className="flex gap-2">
                                <button type="button" onClick={() => setShowLinkModal(false)} className="flex-1 py-3 font-bold text-zinc-500">Cancel</button>
                                <button type="submit" disabled={loading} className="flex-1 py-3 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-bold rounded-xl">{loading ? "..." : "Link Card"}</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
                    <div className="bg-white dark:bg-zinc-900 rounded-3xl p-8 max-w-md w-full shadow-2xl">
                        ...
                        <form onSubmit={handleIssue} className="space-y-4">
                            <input type="number" placeholder="50.00" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800 rounded-xl font-bold" value={issueData.amount} onChange={(e) => setIssueData({ ...issueData, amount: e.target.value })} required />
                            <input type="email" placeholder="customer@example.com" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800 rounded-xl" value={issueData.recipientEmail} onChange={(e) => setIssueData({ ...issueData, recipientEmail: e.target.value })} />
                            <input type="text" placeholder="Internal notes" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800 rounded-xl" value={issueData.notes} onChange={(e) => setIssueData({ ...issueData, notes: e.target.value })} />
                            <div className="flex gap-2">
                                <button type="button" onClick={() => setShowIssueModal(false)} className="flex-1 py-3 font-bold text-zinc-500">Cancel</button>
                                <button type="submit" disabled={loading} className="flex-1 py-3 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-bold rounded-xl">{loading ? "..." : "Issue Card"}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <ConfirmationDialog isOpen={!!confirmResendId} onClose={() => setConfirmResendId(null)} onConfirm={confirmResendAction} title="Resend Gift Card" message="Are you sure you want to resend the gift card email?" confirmText="Resend Email" />
        </div>
    );
}
