import { useLoaderData, useOutletContext, Link, Await } from "react-router";
...
export const loader = async (args: LoaderFunctionArgs) => {
    ...
    const myProgressPromise = apiRequest(`/challenges/my-progress`, token, { headers }).catch(() => []);
    const announcementsPromise = apiRequest(`/community?type=announcement&limit=3`, token, { headers }).catch(() => []);
    const upcomingBookingsPromise = apiRequest(`/bookings/my-upcoming`, token, { headers }).catch(() => []);

    return {
        myProgress: myProgressPromise,
        announcements: announcementsPromise,
        upcomingBookings: upcomingBookingsPromise
    };
}
...
function NextClassSkeleton() {
    return (
        <section className="bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded-2xl p-6 flex items-center gap-5">
            <Skeleton className="h-14 w-14 rounded-xl bg-zinc-300 dark:bg-zinc-700" />
...
function AnnouncementsSkeleton() {
    return (
        <div className="space-y-3">
            {[1, 2].map((i) => (
                <div key={i} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-5 py-4 space-y-2">
...
function ProgressSkeleton() {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[1, 2].map((i) => (
                <div key={i} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 rounded-xl flex items-center gap-4">
...
export default function StudentPortalIndex() {
    ...
    const { myProgress, announcements, upcomingBookings } = data;
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            {/* Next Class Widget */}
            <Suspense fallback={<NextClassSkeleton />}>
                <Await resolve={upcomingBookings}>
                    {(bookings: any) => {
...
// Reverting lucide-material to lucide-react if it doesn't exist
// Wait, I should check the imports in the existing file. Usually it is lucide-react.
