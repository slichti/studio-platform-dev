...
import { Form, useNavigation, useNavigate, Link, useSearchParams, useOutletContext } from "react-router";
...
export default function CreateStudio() {
    ...
    const isE2EBypassed = context?.isE2EBypassed;
    ...
    useEffect(() => {
        ...
        async function checkSlug() {
            ...
            try {
                ...
                const token = isE2EBypassed ? "user_owner_e2e_123" : await (window as any).Clerk?.session?.getToken();
    ...
    if (!isLoaded && !isE2EBypassed) return <div className="p-10 text-center">Loading...</div>;

    const effectiveUser = isE2EBypassed ? {
        primaryEmailAddress: { emailAddress: 'e2e@test.com', verification: { status: 'verified' } }
    } : user;

    const isVerified = effectiveUser?.primaryEmailAddress?.verification?.status === 'verified';
