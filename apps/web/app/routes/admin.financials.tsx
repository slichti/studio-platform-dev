...
export const loader = async (args: any) => {
    ...
    try {
        const preview = await apiRequest("/billing/preview", token);
...
export default function AdminFinancials() {
    ...
    const refreshPreview = async () => {
        setLoading(true);
        try {
            const res = await apiRequest("/billing/preview", token);
            setPreview(res);
            setResult(null);
        } catch (e) {
            console.error(e);
    ...
    const handleCharge = async () => {
        ...
        setLoading(true);
        try {
            const res = await apiRequest("/billing/charge", token, {
                method: "POST"
            });
            setResult(res);
        ...
        <div className="max-w-6xl mx-auto pb-20">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-zinc-500">Review usage and invoice tenants for infrastructure overages.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                    <div className="text-sm font-medium text-zinc-500 mb-1">Projected Overage Revenue</div>
                ...
                <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                    <div className="text-sm font-medium text-zinc-500 mb-1">Billable Tenants</div>
                ...
                <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                    <div className="text-sm font-medium text-zinc-500 mb-1">Unit Economies</div>
                    <div className="text-sm space-y-1 mt-2">
                        <div className="flex justify-between"><span className="text-zinc-500">SMS</span> <span>$0.0075</span></div>
                        <div className="flex justify-between"><span className="text-zinc-500">Email</span> <span>$0.0006</span></div>
                        <div className="flex justify-between"><span className="text-zinc-500">Stream</span> <span>$0.05/min</span></div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-zinc-200 bg-zinc-50 flex justify-between items-center">
                    <h3 className="font-semibold text-zinc-900">Chargeback Preview</h3>
                    <span className="text-xs text-zinc-500">Based on current usage vs limits</span>
                </div>
                ...
                {tenants.length === 0 ? (
                    <div className="p-12 text-center text-zinc-500">
                        ...
                        <p className="max-w-sm mx-auto mt-1">No tenants are currently exceeding their usage limits. Revenue projection is $0.00.</p>
                    </div>
                ) : (
                    <table className="w-full text-left">
                        <thead className="bg-zinc-50 border-b border-zinc-200">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Breakdown</th>
                                <th className="px-6 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-right">Total Overage</th>
                            </tr>
                        </thead>
            ...
            <div className="mt-8 p-4 bg-blue-50 text-blue-800 rounded-lg text-sm border border-blue-100 flex gap-3">
                ...
                <div>
                    <strong>How Chargebacks Work:</strong>
                    <ul className="list-disc list-inside mt-1 space-y-1 opacity-90 ml-1">
                        <li>Invoices are generated as "Pending Invoice Items" in Stripe.</li>
                        <li>They will be included in the tenant's next normally scheduled invoice (if subscribed) or can be triggered immediately if configured.</li>
                        <li>Tenants with "Billing Exempt" status are excluded from this list automatically.</li>
                        <li>You can manually adjust limits in the Tenants page to waive overages before charging.</li>
                    </ul>
                </div>
            </div>
        </div>
