...
*   **Studio Management:** Class scheduling, member management, point-of-sale (with automatic sales tax, rich transaction metadata, and robust refund tracking), and Kiosk Mode with walk-in booking support. POS now supports seamless membership sign-ups and auto-account creation for new customers. **Instructors** get a dedicated _My Teaching Schedule_ view in the portal and optional tools to view rosters, register students, and check-in attendees (studio-configurable).
*   **Memberships & Subscriptions:** Self-service membership plans with Stripe billing, free trials, archive/restore lifecycle, and student-facing plan browser. Pricing is modeled as **price per interval** (e.g. `$125 / month`) with optional **interval counts** (e.g. 3 months) so fixed-term passes bill correctly and end after the N‑th period when auto‑renew is off. Students can cancel or manage subscriptions directly from their profile, and Stripe Billing Portal is wired for payment method updates. When memberships or class packs are purchased or admin-assigned, users receive detailed **transactional emails**, and studios can target follow-ups via automation triggers (`pack_purchased`, enriched `membership_started` with `source`).
...
*   **Automations & Email Marketing:** Powerful email and SMS marketing automations with trigger events (Waitlist Promoted, Pack Credits Low, Birthday, Absent, Trial Ending, **Pack Purchased/Assigned**, Membership Started). Full multi-tenant Email Marketing system allowing studios to attach custom domains, sync newsletter audiences, and send HTML broadcasts.
*   **Community Hub**: A studio-wide engagement platform with **Isolated Topics** (e.g., # Nature, # Yoga Tips). Supports **Eligibility Rules** (auto-granting topic access based on course enrollment or membership tier) and **Hidden Topics**. Includes **Isolated Feeds**, **AI-Powered Writing Assistance** (Gemini AI), and an **Optimized Centered Layout** for balanced readability on large displays. Full post management suite includes editing, deletion, and topic migration.
*   **Analytics Dashboards:** Comprehensive reporting with churn-risk modeling, revenue breakdown (subscriptions vs packs vs POS), and automation effectiveness tracking. Includes **Automated Billing Sync** with immediate Stripe invoice finalization, platform commission tracking, and email receipts for tenant owners.
*   **Platform Monetization:** Configurable platform fee system (commissions) managed via Admin UI. Supports global tier fees (basis points) and per-tenant overrides for bespoke billing logic. Fees are automatically collected during POS and Membership checkouts.
*   **SEO Management:** Automated multi-tenant localized SEO via edge-rendered HTMLRewriter, streaming D1 sitemaps, and dynamic Schema.org injection. Includes Google Business Profile OAuth sync, real-time Google Indexing API integration, and **AI-Powered Local Blogging** via Gemini AI. Platform and studio admins can configure a **Google Review Link** and send email/SMS review requests directly from SEO tooling and Reviews dashboards; incoming Google Reviews surface in-app with **AI reply drafts** to accelerate moderation. Advanced capabilities include **Strategic Strategy Guidance**, **Competitive Search Ranking**, **Automated Student Push Notifications**, and **Advanced AI Optimization** with granular **feature-specific model overrides** and precise **Usage/Cost Tracking** (tokens and models) across all AI features (Blog, Reviews, Email, Community).
*   **QR Codes:** Generate tracking-ready QR codes for check-in, app downloads, and marketing.
*   **Member Tags:** Platform- and tenant-scoped member tags (e.g. Senior, Silver Sneakers, VIP) with default tag sets created for every tenant, optional discounts, and eligibility hooks for features like tag-restricted classes and targeted automations.
...
Key UX flows include:

*   **Class Schedule** with **Calendar / Tile / List** views, a unified **Show previous classes** toggle, and admin‑only **Bulk Operations** (bulk schedule, reschedule, cancel) for high-volume studios.
*   **Roster Management** with quick Add Student modal supporting bulk-add (keep open until Done) and streamlined check‑in, notes, and waitlist promotion.
